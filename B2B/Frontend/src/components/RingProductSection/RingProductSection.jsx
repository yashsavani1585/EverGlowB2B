...
//   const { items: products = [], loading } = useCategoryProducts("rings");
...
//       {/* Remaining Products (guard against short lists) */}
//       {[ [12,15], [15,18], [18,21] ].map(([a,b], i) => (
//         <div key={i} className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
//           {products.slice(a, b).map((p) => (
//             <ProductCard key={p.id || p._id} product={p} />
//           ))}
//         </div>
//       ))}
...
// export default RingProductSection;
...
const RingProductSection = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-10 space-y-10">
      {/* First 3 Products */}
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
        {memoizedProducts.slice(0, 3).map((p) => (
      ...
      {/* Next 3 Products */}
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
        {memoizedProducts.slice(3, 6).map((p) => (
          <ProductCard key={p._id || p.id} product={p} />
        ))}
      </div>
      ...
      {/* 3 More Products */}
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
        {memoizedProducts.slice(6, 9).map((p) => (
          <ProductCard key={p._id || p.id} product={p} />
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-stretch py-12">
        <div className="flex flex-col gap-6">
          {memoizedProducts[0] && <ProductCard product={memoizedProducts[0]} />}
          {memoizedProducts[1] && <ProductCard product={memoizedProducts[1]} />}
        </div>
        <div className="md:col-span-2 relative rounded-2xl overflow-hidden">
          <img
            src={PromotionalBanner3}
      ...
      {/* More Product Grids */}
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
        {memoizedProducts.slice(9, 12).map((p) => (
          <ProductCard key={p._id || p.id} product={p} />
        ))}
      </div>
      ...
      {[ [12,15], [15,18], [18,21] ].map(([a,b], i) => (
        <div key={i} className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          {memoizedProducts.slice(a, b).map((p) => (
            <ProductCard key={p._id || p.id} product={p} />
          ))}
        </div>
      ))}
