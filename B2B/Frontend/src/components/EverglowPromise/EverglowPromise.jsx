...
const EverglowPromise = () => {
  ...
  return (
    ...
    >
      ...
      >
        {/* Left Heading */}
        <div className="hidden md:block w-full md:w-1/3 text-center md:text-left relative">
          <h3
          ...
          <div className="hidden md:block absolute top-[55%] right-0 w-28 border-t-2 border-yellow-700"></div>
        </div>

        {/* Mobile heading */}
        <h4
          className="block md:hidden text-3xl font-serif font-bold text-black leading-tight text-center"
          style={{ fontFamily: "'Garamond', serif" }}
        >
          GEMS GLOBAL Promise
        </h4>
        ...
        {/* Right Grid */}
        <div className="w-full md:w-2/3 border-2 border-black rounded-md p-6 md:p-10">
          <div className="grid grid-cols-3 sm:grid-cols-3 md:grid-cols-3 gap-6 sm:gap-8 md:gap-10 text-center">
            {promises.map((item, index) => (
              ...
              >
                ...
                <p className="mt-3 text-xs sm:text-sm md:text-base font-medium text-black">
                  {item.text}
                </p>
              </motion.div>
            ))}
