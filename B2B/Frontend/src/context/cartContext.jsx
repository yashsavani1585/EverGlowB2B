...
const authHeader = () => ({ headers: { token: localStorage.getItem("token") || "" } });
...
export const CartProvider = ({ children }) => {
  ...
  const flattenServerCart = useCallback(async (cartData) => {
    ...
    ids.forEach((pid) => {
      ...
      Object.keys(byColor).forEach((ckey) => {
        const qty = Number(byColor[ckey] || 0);
        if (qty <= 0) return;
  ...
  const mergeOneLocal = useCallback((pid, qty, color, productObj) => {
    ...
    setItems((prev) => {
      ...
      if (i > -1) {
        ...
        next = [...prev, { key, product: productObj || { _id: pid }, qty: qty || 1, color: color || null }];
      }
      recalc(next);
      return next;
    });
  ...
  const add = useCallback(async (productId, qty = 1, color = null) => {
    if (!isAuthenticated()) return false;
    const c = normColorKey(color);
    try {
      await axios.post(`${API}/cart/add`, {
  ...
  }, [API, fetchProductDoc, mergeOneLocal]);

  const update = useCallback(async (key, qty) => {
    if (!isAuthenticated()) return false;
    ...
    try {
      ...
      setItems((prev) => {
        const next = prev.map((x) => (x.key === key ? { ...x, qty: Number(qty) } : x));
        // remove if 0
